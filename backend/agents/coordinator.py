...
from backend.agents.mr_graham import run_graham_analysis
...
async def run_analysis(
    ...
    """
    # 1. Run analyst reports in sequence
    ...
    # 2. Run Mr. Graham — Benjamin Graham value investing screen
    print(f"[{ticker}] Running Mr. Graham (Value Investing Screen)...")
    graham_report = await run_graham_analysis(ticker, fundamentals)

    # 3. Run researcher debate loop — Graham report is included as additional context
    ...
    # 4. Run final Trader decision maker — Graham report enriches the final synthesis
